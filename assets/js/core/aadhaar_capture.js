        ...
        onOpen: function() {
            ...
            discover_device((data, rd_error_bag) => {
                ...
                if (error == 0) {
                    ...
                    capture_fingerprint(url + deviceCapturePath, data => {
                        console.log(data);
                        if (error != 0) {
                            this.setContent(`
                                <div class="text-center">
...
function capture_fingerprint(url, callback) {
    // S - Staging
    // PP - Pre-Production
    // P - Production
    var env = 'P';

    var data = `<?xml version="1.0"?><PidOptions ver="1.0"><Opts fCount="1" fType="0" iCount="0" pCount="0" format="0" pidVer="2.0" timeout="10000" posh="UNKNOWN" env="${env}" /><CustOpts></CustOpts></PidOptions>`;
        ...
        success(data) {
            ...
            if (errCode == 0) {
                ...
                if (!$.isXMLDoc(data)) {
                    ...
                    $('[name="txtPidData"]').val("<PidData>"+$(newdata).find('PidData').html()+"</PidData>");
                }
