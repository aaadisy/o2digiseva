@extends('layouts.app')
@section('title', "Cash Deposit Statement")
@section('pagetitle',  "Cash Deposit Statement")

@php
    $table = "yes";
    $export = "cashdepositbillpay";
        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="panel-title">Cash Deposit Statement</h4>
                </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        var url = "{{url('statement/fetch')}}/cashdepositbillpaystatement/{{$id}}";
        var onDraw = function() {
        };
        var options = [
            { "data" : "name",
            ...
            { "data" : "bank",
                render:function(data, type, full, meta){
                    return `Account Number - `+full.aadhar+`<br>Bank Name - `+full.bank+`<br>IFSC - `+full.mobile+`<br>Bank Account Holder - `+full.test;
                }
            },
            { "data" : "bank",
                render:function(data, type, full, meta){
                    return `Name - `+full.test+`<br>Due Date - `+full.user_idew;
                }
            },
            { "data" : "bank",
                render:function(data, type, full, meta){
            ...
            { "data" : "bank",
                render:function(data, type, full, meta){
                    return `Amount - <i class="fa fa-inr"></i> `+full.amount+`<br>Surcharge - <i class="fa fa-inr"></i> `+full.charge+`<br>GST - <i class="fa fa-inr"></i> `+full.gst+`<br>TDS - <i class="fa fa-inr"></i> `+full.tds;
                }
            },
            { "data" : "status",
                render:function(data, type, full, meta){
    ...
    function viewUtiid(id){
        $.ajax({
            url: `{{url('statement/fetch')}}/cashdepositbillpaystatement/`+id,
            type: 'post',
            headers: {
