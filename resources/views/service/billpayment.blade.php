@extends('layouts.app')
 @if($type == 'cashdeposit') 
 @section('title', 'Cash Deposit / Express Payout')
@section('pagetitle', 'Cash Deposit / Express Payout')
 @else 
...
<div class="content">
    <div class="row">
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h4 class="panel-title">
                        @if($type == 'cashdeposit') Cash Deposit / Express Payout @else {{ucfirst($type)}} Bill Payment @endif
                    </h4>
                </div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label>@if($type == 'cashdeposit') Select Bank @else {{ucfirst($type)}} Operator @endif </label>
                                <input type="hidden" name="billtype" value="{{ $type }}" />
                                <select name="provider_id" class="form-control select" required="">
                                    <option value="">@if($type == 'cashdeposit') Select Bank @else Select Operator @endif</option>
                            ...
                            <div class="form-group col-md-4">
                                <label>@if($type == 'cashdeposit') Bank Account Number @elseif($type == 'insurance') Policy Number @elseif($type == 'electricity') Consumer ID @else  Account Number @endif
                                 </label>
                                <input type="text" name="number" class="form-control" placeholder="@if($type == 'cashdeposit') Bank Account Number @elseif($type == 'insurance') Policy Number @elseif($type == 'electricity') Consumer ID @else  Account Number @endif" required="">
                            </div>
                            <div class="form-group col-md-4">
                                <label>@if($type == 'cashdeposit') Bank IFS Code @else  Biller Mobile Number @endif</label>
                                <input type="text" name="mobile" class="form-control" placeholder="Enter @if($type == 'cashdeposit') Bank IFS Code @else  Biller Mobile Number @endif number" required="">
                            </div>
                        </div>
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label>@if($type == 'cashdeposit') Bank Account Holder Name @else  Name @endif </label>
                                <input type="text" name="biller" class="form-control" placeholder="@if($type == 'cashdeposit') Bank Account Holder Name @else  Name @endif" required="" @if($type == 'cashdeposit')  @elseif ($type == 'gamingtopup')  @else readonly="" @endif>
                            </div>
                            
                            
                            <div class="form-group col-md-4">
                                <label>@if($type == 'cashdeposit') Date @else Bill Date @endif</label>
                                <input  @if($type == 'cashdeposit') type="date"  @elseif ($type == 'gamingtopup') type="date" @else type="text" @endif name="duedate" class="form-control" placeholder="Enter @if($type == 'cashdeposit') Date @else Bill Date @endif" required=""  @if($type == 'cashdeposit')  @elseif ($type == 'gamingtopup')  @else readonly="" @endif>
                            </div>
                            <div class="form-group col-md-4">
                                <label>@if($type == 'cashdeposit') Amount @else Bill Amount @endif</label>
                                <input type="text"   name="amount" class="form-control" placeholder="@if($type == 'cashdeposit') Amount @else Bill Amount @endif" @if($type == 'cashdeposit')  @elseif ($type == 'gamingtopup')  @else readonly="" @endif >
                            </div>
                            <div class="form-group col-md-4">
        ...
        <div class="col-sm-12">
            <div class="panel panel-default">
                <div class="panel-heading">
          <h4 class="panel-title">Recent  @if($type == 'cashdeposit') Cash Deposit / Express Payout @else {{ucfirst($type)}} Bill Payment @endif</h4>
        </div>
        <div class="panel-body">
        </div>
              ...
              <div class="panel-body">
                  <div class="clearfix">
                      ...
                      <div class="pull-right">
                         @if(Request()->segment(2) == 'cashdeposit')
                          <h4>Cash Deposit Invoice</h4>
                          @else
                ...
                form.ajaxSubmit({
                    ...
                    success:function(data){
                        ...
                        if(data.status == "success" || data.status == "pending"){
                            ...
                            notify(data.status+ "! "+data.description, 'warning');
                        }
                    },
        ...
        if(operator != "" && number != "" && mobile != ""){
            ...
            .done(function(data) {
                ...
                if(data.statuscode == "TXN"){
                    if(data.data.operator == "LICOF"){
                        $('#billpayForm').find('[name="duedate"]').val(data.data.Duedatefromto);
                        $('#billpayForm').find('[name="amount"]').val(data.data.Netamount);
                    }
